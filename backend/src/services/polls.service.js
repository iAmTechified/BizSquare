...
class PollsService {
     ...
     */
    static async getActivePolls(userId) {
        const { rows } = await pool_1.pool.query(`SELECT p.* 
     ...
     */
    static async submitSwipe(userId, pollId, response) {
        const { rows } = await pool_1.pool.query(`INSERT INTO user_poll_responses (user_id, poll_id, response)
       VALUES ($1, $2, $3)
       ON CONFLICT (user_id, poll_id) 
       DO UPDATE SET response = EXCLUDED.response, created_at = CURRENT_TIMESTAMP
       RETURNING *`, [userId, pollId, response]);
        return rows[0];
    }
}
